...
void mmc_rescan(struct work_struct *work)
{
	...
	if((i == ARRAY_SIZE(freqs)) || (freqs[i] < host->f_min))
	{
		/*sd card initialize failed , mark sdcard removed*/
		if (!(host->caps & MMC_CAP_NONREMOVABLE)){
			if(host->is_sdcard_removed == false)
			{
				pr_info("%s: sdcard doesn't exist , mark sdcard as removed\n", mmc_hostname(host));
				host->is_sdcard_removed = true;
			}
		}
	}
