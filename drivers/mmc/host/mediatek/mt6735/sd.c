...
static void msdc_ops_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (host->power_mode != ios->power_mode) {
		...
		case MMC_POWER_OFF:
		case MMC_POWER_UP:
			//modified by xiongping start to implement sdcard immediately power off functionality
			if(host->hw->host_function == MSDC_SD)
			{
				pr_info("%s:%s  ios->power_mode = %s\n",mmc_hostname(mmc),__func__, (ios->power_mode==MMC_POWER_UP)?"MMC_POWER_UP":"MMC_POWER_OFF");
				if((ios->power_mode == MMC_POWER_UP) && (mmc->is_sdcard_removed == false))
				{
					pr_info("%s: MMC_POWER_UP mode , enable LDO\n",mmc_hostname(mmc));
					gpio_set_value(83, 1);
				}
				if(ios->power_mode == MMC_POWER_OFF)
				{
					pr_info("%s: MMC_POWER_OFF mode , disable LDO\n",mmc_hostname(mmc));
					gpio_set_value(83, 0);
				}			
			}
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	if (host->hw->host_function == MSDC_SD)
	{
		pr_debug("disable LDO , its enabled by default.\n");
		gpio_set_value(83, 0);
	}
	//modified by xiongping end 
