...
static bool power_check_flag = 0;//wkh add
...
*/
#endif
u8 gt9xx_sensor_id = 0xff;//add by wkh
...
static struct proc_dir_entry *gt91xx_info_proc;
...
//add by wkh begin
static ssize_t goodix_get_gesture_wakeup(struct device *dev, struct device_attribute *attr, char *buf)
{
    //struct goodix_ts_data *drvdata= dev_get_drvdata(dev);
    int enable = enable_gesture_wakeup;

    //return snprintf(buf, sizeof(enable) + 2, "%d\n", enable);
	return sprintf(buf, "%d\n", enable);
}

static ssize_t goodix_set_gesture_wakeup(struct device *dev, struct device_attribute *attr,
                             const char *buf, size_t size)
{
	...
	if (ret != 1) {
		GTP_ERROR("[goodix]: Invalid values\n");
		return -EINVAL;
	}
    #endif

	ret = kstrtoul(buf, 0, &enable);
    enable_gesture_wakeup = (int)enable;
...
static struct device_attribute *gt9xx_attrs[] = {
	...
	&dev_attr_enable_gesture_wakeup,
#if GTP_SCP_GESTURE_WAKEUP
...
//add by wkh begin
static ssize_t gt91xx_info_read_proc(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	char *page = NULL;
	char *ptr = NULL;
	int /*i, */len, err = -1,ret = 0;
	...
	//ptr += sprintf(ptr, "IC VERSION: \n");
	if (buf[5] == 0x00) {
		//GTP_INFO("IC VERSION: %c%c%c%c_%02x%02x", buf[2], buf[3], buf[4], buf[5], buf[7], buf[6]);
		ptr += sprintf(ptr,"IC VERSION: %c%c%c%c_%02x%02x\n",buf[2], buf[3], buf[4], buf[5], buf[7], buf[6]);
	} else {
		if (buf[5] == 'S' || buf[5] == 's')
			//chip_gt9xxs = 1;
		//GTP_INFO("IC VERSION:%c%c%c%c_%02x%02x", buf[2], buf[3], buf[4], buf[5], buf[7], buf[6]);
	...
	ptr += sprintf(ptr,"firmware VERSION: %02x%02x\n",buf[7], buf[6]);
	
	//module info
	switch (gt9xx_sensor_id){
	case 0x0:
		ptr += sprintf(ptr,  "module,xinli(0x%x)\n", grp_cfg_version);//xinli
		break;
	case 0x1:	
		ptr += sprintf(ptr,  "module,lianchuang(0x%x)\n", grp_cfg_version);//lianchuang
		break;
	default:
...
static ssize_t gt91xx_info_write_proc(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
	GTP_DEBUG("write count %ld\n", (unsigned long)count);
    return count;
}
...
static s32 gtp_init_panel(struct i2c_client *client)
{
    ...
    gt9xx_sensor_id=sensor_id;//add by wkh
...
//add by wkh begin
static const struct file_operations gt_info_proc_fops = {
	.write = gt91xx_info_write_proc,
	.read = gt91xx_info_read_proc,
};
//end
#ifdef CONFIG_OF_TOUCH
...
static int tpd_registration(void *client)
{
	...
	gt91xx_config_proc = proc_create(GT91XX_CONFIG_PROC_FILE, 0664, NULL, &gt_upgrade_proc_fops);
	if (gt91xx_config_proc == NULL)
		GTP_ERROR("create_proc_entry %s failed\n", GT91XX_CONFIG_PROC_FILE);
    //add by wkh begin
	gt91xx_info_proc = proc_create(GT91XX_INFO_PROC_FILE, 0664, NULL, &gt_info_proc_fops);
	if (gt91xx_info_proc == NULL)
		GTP_ERROR("create_info_proc_entry %s failed\n", GT91XX_INFO_PROC_FILE);
	//end
	...
	input_set_capability(tpd->dev, EV_KEY, KEY_GESTURE_DOUBLEC);
	...
	__set_bit(KEY_GESTURE_DOUBLEC, tpd->dev->keybit);
...
static int touch_event_handler(void *unused)
{
	...
	do {
		...
		//mutex_lock(&i2c_access);wkh
#ifdef CONFIG_GTP_CHARGER_SWITCH
		gtp_charger_switch(0);
#endif
#if GTP_SLIDE_WAKEUP
		if (/*DOZE_ENABLED == doze_status*/DOZE_DISABLED != doze_status && enable_gesture_wakeup) {
			...
			if (ret > 0) {
                ...
                (doze_buf[2] == 0x5E)*/(doze_buf[2] == 0xAB) || (doze_buf[2] == 0xBA))
				{
					/*if (doze_buf[2] != 0x5E)
                {
			...
			{
			    GTP_INFO("C to light up the screen!");
					doze_status = DOZE_WAKEUP;
			...
			{
			    GTP_INFO("m to light up the screen!");
					doze_status = DOZE_WAKEUP;
			...
			{
			    GTP_INFO("W to light up the screen!");
					doze_status = DOZE_WAKEUP;
			...
			{
			    GTP_INFO("o to light up the screen!");
					doze_status = DOZE_WAKEUP;
					...
					doze_status = DOZE_WAKEUP;
					input_report_key(tpd->dev, KEY_GESTURE_DOUBLEC, 1);
					input_sync(tpd->dev);
					input_report_key(tpd->dev, KEY_GESTURE_DOUBLEC, 0);
					input_sync(tpd->dev);
					/* clear 0x814B*/
					doze_buf[2] = 0x00;
					gtp_i2c_write(i2c_client_point, doze_buf, 3);
...
#endif
    //GTP_DEBUG("wkh,GTP enter gtp_enter_sleep.%d\n",enable_gesture_wakeup);
//#ifdef CONFIG_GTP_POWER_CTRL_SLEEP
    if(enable_gesture_wakeup == 0){
	GTP_GPIO_OUTPUT(GTP_RST_PORT, 0);
	GTP_GPIO_OUTPUT(GTP_INT_PORT, 0);
	msleep(20);
...
//#ifdef CONFIG_GTP_POWER_CTRL_SLEEP
    if(power_check_flag)/*enable_gesture_wakeup == 0)*/{
#if GTP_COMPATIBLE_MODE
	if (CHIP_TYPE_GT9F == gtp_chip_type) {
		force_reset_guitar();
...
static void tpd_suspend(struct device *h)
{
	...
	tpd_enter_doze();
#elif GTP_SLIDE_WAKEUP
    if(enable_gesture_wakeup){
	    gtp_enter_doze(i2c_client_point);
	    GTP_DEBUG("GTP enter doze.");
	}
	else
	{
	    disable_irq(touch_irq);
		//GTP_ERROR("wkh tpd_suspend disable_irq\n");
