				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		val->intval = battery_meter_get_battery_current_now();//data->BAT_CurrentNow;
		break;
		/* Dual battery */
...
static void mt_battery_update_EM(struct battery_data *bat_data)
{
	...
	bat_data->BAT_CurrentNow = battery_meter_get_battery_current_now();//BMT_status.CurrentNow;
	/* Dual battery */
...
{
	static unsigned int timer_counter;
	static bool ui_soc_inited = KAL_FALSE;
	...
	if ((BMT_status.UI_SOC > BMT_status.SOC) && ((BMT_status.UI_SOC != 1))) {
		...
		if((BMT_status.SOC != 100)
			|| ((BMT_status.SOC == 100) && (BMT_status.bat_charging_state == CHR_BATFULL))){
			battery_log(BAT_LOG_CRTI, "[Sync_Real] UI_SOC=%d, SOC=%d\n",BMT_status.UI_SOC, BMT_status.SOC);
			BMT_status.UI_SOC = BMT_status.SOC;
		}
		else if(ui_soc_inited == KAL_FALSE){
			BMT_status.UI_SOC = BMT_status.SOC;
			battery_log(BAT_LOG_CRTI, "[Sync_Real] init UI_SOC=%d, SOC=%d\n",BMT_status.UI_SOC, BMT_status.SOC);
			ui_soc_inited = KAL_TRUE;
		}
#else
...
{
#if 0
	static int init_temperature = 35;
	...
	BMT_status.CurrentNow = battery_meter_get_battery_current_now();
	...
	#if defined(TEMPERATURE_ADJUST)
	if(BMT_status.temperature > 50 && BMT_status.temperature <= 60)
		BMT_status.temperature = BMT_status.temperature - 2;
	else if(BMT_status.temperature > 40 && BMT_status.temperature <= 50)
		BMT_status.temperature = BMT_status.temperature - 1;
	#endif
	...
	#if 0
	init_temperature += direction;
	if(init_temperature >= (MAX_CHARGE_TEMPERATURE+2))
	{
		direction = -1;
	}
	if(init_temperature <= (MIN_CHARGE_TEMPERATURE-2))
	{
		direction = 1;
	}
	BMT_status.temperature = init_temperature;
	#endif
...
static void mt_battery_notify_VBatTemp_check(void)
{
	...
	//if (BMT_status.temperature >= batt_cust_data.max_charge_temperature) {
	if ((BMT_status.temperature >= batt_cust_data.max_charge_temperature) && BMT_status.charger_exist) {
	...
	//else if (BMT_status.temperature < MIN_CHARGE_TEMPERATURE) {
	else if ((BMT_status.temperature <= batt_cust_data.min_charge_temperature) && BMT_status.charger_exist) {
		g_BatteryNotifyCode |= 0x0020;
		battery_log(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) out of range(too low)\n",
			    BMT_status.temperature);
	}
		...
		if(g_BatteryNotifyCode & 0x0002)
			g_BatteryNotifyCode &= ~(0x0002);
		if(g_BatteryNotifyCode & 0x0020)
			g_BatteryNotifyCode &= ~(0x0020);
		battery_log(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d)\n",
			    BMT_status.temperature);
	}
...
{
	ktime_t ktime;
	unsigned int hv_voltage = 6000000;//batt_cust_data.v_charger_max * 1000;
