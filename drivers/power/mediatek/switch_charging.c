...
void select_charging_current(void)
{
	if (g_ftm_battery_flag) {
		...
		if (BMT_status.charger_type == STANDARD_HOST) {
			...
			#ifdef COOL_CHARGE_TEMPERATURE
			if(BMT_status.temperature <= COOL_CHARGE_TEMPERATURE)
				g_temp_CC_value = AC_CHARGER_CURRENT_BATT_COOL;
			else
			#endif
			g_temp_CC_value = batt_cust_data.ac_charger_current;

			if(BMT_status.bat_vol >= 4200)
...
#endif
	printk("[mt_battery_charging_algorithm] bat_charging_state = 0x%x\n",BMT_status.bat_charging_state);
