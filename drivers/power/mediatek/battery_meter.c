...
void dod_init(void)
{
	...
	pr_warn("[FGADC] get_hw_ocv=%d, HW_SOC=%d, SW_SOC = %d\n",
	...
	if ((abs(gFG_capacity_by_v_init - gFG_capacity_by_v) > 5)){
		gFG_capacity_by_v = gFG_capacity_by_v_init;
		pr_warn("[FGADC] gFG_capacity_by_v = gFG_capacity_by_v_init = %d\n",
		 	gFG_capacity_by_v);
	}
#endif
...
#else
	g_rtc_fg_soc = get_rtc_spare_fg_value();
	pr_warn("[FGADC]g_rtc_fg_soc = %d, get_rtc_spare_fg_value\n",
		g_rtc_fg_soc);
#endif
...
#if defined(SOC_BY_HW_FG)
	pr_warn("[FGADC] g_rtc_fg_soc=%d, gFG_capacity_by_v=%d, cust_poweron_delta_capacity_tolrance = %d, gFG_capacity_by_v_init = %d\n",
		g_rtc_fg_soc, gFG_capacity_by_v, batt_meter_cust_data.cust_poweron_delta_capacity_tolrance, gFG_capacity_by_v_init);
#if defined(INIT_SOC_BY_SW_SOC)
if (((g_rtc_fg_soc != 0)
	     &&
	     (((abs(g_rtc_fg_soc - gFG_capacity_by_v)) <=
...
if (((g_rtc_fg_soc != 0)
	...
	{
		gFG_capacity_by_v = g_rtc_fg_soc;
		pr_warn("[FGADC] gFG_capacity_by_v = g_rtc_fg_soc = %d\n",
		gFG_capacity_by_v);
	...
	pr_warn("[FGADC] g_rtc_fg_soc=%d, gFG_capacity_by_v=%d\n",
		 g_rtc_fg_soc, gFG_capacity_by_v);

	if (gFG_capacity_by_v == 0 && bat_is_charger_exist() == KAL_TRUE) {
		gFG_capacity_by_v = 1;

		pr_warn("[FGADC] gFG_capacity_by_v=%d\n", gFG_capacity_by_v);
	}
	...
	if (batt_meter_cust_data.change_tracking_point) {
		...
		pr_warn("[FGADC] gFG_15_vlot = %dmV\n", gFG_15_vlot);
	} else {
		/* gFG_15_vlot = fgauge_read_v_by_capacity(86); //14% */
		gFG_15_vlot = fgauge_read_v_by_capacity((100 - g_tracking_point));
		pr_warn("[FGADC] gFG_15_vlot = %dmV\n", gFG_15_vlot);
		if ((gFG_15_vlot > 3800) || (gFG_15_vlot < 3600)) {
			pr_warn("[FGADC] gFG_15_vlot(%d) over range, reset to 3700\n",
				 gFG_15_vlot);
			gFG_15_vlot = 3700;
		}
	}
...
/* 1. Get Raw Data */
	...
	pr_warn("[FGADC] SWOCV : %d,%d,%d,%d,%d,%d\n",
...
/* 4. update DOD0 */
	pr_warn("[FGADC] gFG_capacity_by_v = %d, gFG_capacity_by_v_init = %d, gFG_capacity_by_c = %d\n",
		 gFG_capacity_by_v, gFG_capacity_by_v_init, gFG_capacity_by_c);
...
#endif
/* 5. Logging */
	pr_warn("[FGADC] %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\r\n",
		 gFG_Is_Charging, gFG_current, gFG_columb, gFG_voltage, gFG_capacity_by_v,
		 gFG_capacity_by_c, gFG_capacity_by_c_init, gFG_BATT_CAPACITY,
		 gFG_BATT_CAPACITY_aging, gFG_compensate_value, gFG_ori_voltage,
		 batt_meter_cust_data.ocv_board_compesate, batt_meter_cust_data.r_fg_board_slope,
...
signed int battery_meter_get_battery_current_now(void)
{
	...
	fg_current_inout_battery = 100 * fg_current_inout_battery; //unit: uA

	bm_print(BM_LOG_FULL, "current_now : %duA\n", fg_current_inout_battery);
