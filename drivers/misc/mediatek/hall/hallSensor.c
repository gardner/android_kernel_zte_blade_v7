...
//extern void synaptics_touchscreen_gloved_finger(bool enable); 
...
static void hall_work_func(struct work_struct * work) 
{
	...
	printk("hall ic enable is %d\n",hall_ic_data->enabled);
	printk("hall report is %d\n",report_en);
	if((hall_ic_data->enabled==1)&&(1==report_en)){
		if(value==1){
			//synaptics_touchscreen_gloved_finger(false);
			...
			//synaptics_touchscreen_gloved_finger(true);
