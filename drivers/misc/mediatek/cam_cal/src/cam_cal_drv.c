		...
		for (i = 0; pCamCalList[i].sensorID != 0; i++) {
			...
			if (pCamCalList[i].sensorID == sensorID) {
				...
				if (cam_cal_get_i2c_client(g_i2c_info[g_curDevIdx], &(cmdInfo->client))) {
					...
					CAM_CALDB("pCamCalFunc[%d].cmdType = %d,pCamCalList[%d].cmdType = %d\n",j, pCamCalFunc[j].cmdType, i,pCamCalList[i].cmdType);
...
#endif
{
	...
	//	g_lastDevID = ptempbuf->deviceID;
		cam_cal_set_i2c_bus(0);
	//}

	pcmdInf = cam_cal_get_cmd_info_ex(0);
	...
	case CAM_CALIOC_G_READ:
		...
		//if (g_lastDevID != ptempbuf->deviceID) {
			//g_lastDevID = ptempbuf->deviceID;
			cam_cal_set_i2c_bus(SENSOR_DEV_MAIN);
		//}

		pcmdInf = cam_cal_get_cmd_info_ex(0xd853);
