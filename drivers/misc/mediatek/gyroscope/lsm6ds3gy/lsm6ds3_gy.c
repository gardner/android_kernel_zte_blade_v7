...
static struct gyro_hw *hw = &lsm6ds3_gyro_cust;
...
//static struct i2c_board_info __initdata i2c_lsm6ds3_gyro={ I2C_BOARD_INFO(LSM6DS3_GYRO_DEV_NAME, 0x6A)}; //0xD4>>1 is right address
...
static int LSM6DS3_gyro_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	if(true == enable)
	{
		...
		GYRO_LOG("LSM6DS3_gyro set power mode: ODR %d!\n", obj->sample_rate);
	}
	else
	{
		// do nothing
...
static int LSM6DS3_gyro_init_client(struct i2c_client *client, bool enable)
{
	...
	res = LSM6DS3_gyro_SetSampleRate(client, obj->sample_rate);
	if(res != LSM6DS3_SUCCESS ) 
	{
		return res;
	}
...
static int lsm6ds3_gyro_enable_nodata(int en)
{
	...
	{
		enable_status = false;
		priv->sample_rate = LSM6DS3_GYRO_ODR_104HZ; 
	}
...
//#if 0
	...
	if(priv == NULL)
	{
     ...
     GYRO_LOG("lsm6ds3_gyro_set_delay (%d)\n",value);
//#endif
    return 0;
}
...
static int lsm6ds3_gyro_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->sample_rate = LSM6DS3_GYRO_ODR_104HZ;
