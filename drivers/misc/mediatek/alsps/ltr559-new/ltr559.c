...
//add start by jiangchong for prox sensor calibration,2015\12\09
#include <asm/atomic.h>
#include <linux/proc_fs.h>
#include <linux/fs.h>
//add end by jiangchong for prox sensor calibration,2015\12\09
...
//add start by jiangchong for prox sensor calibration,2015\12\09
static bool ps_calibrating = false;
static u16 prox_threshold_startup_mean = 0;
static u16 prox_factory_saved_value = 0;
static u16 prox_threshold_hi_param = 0;
static u16 prox_threshold_lo_param = 0;
ssize_t alsps_read_proc(struct file *filp, char *buff, size_t len, loff_t *off);
ssize_t alsps_write_proc(struct file *filp, const char *buff, size_t len, loff_t *off);
//add end by jiangchong for prox sensor calibration,2015\12\09
...
struct ltr559_priv {
    ...
    struct mutex lock;
//add start by jiangchong for prox sensor calibration,2015\12\09    
    struct workqueue_struct *adjust_wqueue; 
    struct delayed_work prox_adjust_work;
//add end by jiangchong for prox sensor calibration,2015\12\09
	/*i2c address group*/
...
static int ltr559_als_read_raw(struct i2c_client *client, u16* data, u16* Ch0, u16* Ch1, u16* Ratio)
{
...
static ssize_t ltr559_show_raw_als(struct device_driver *ddri, char *buf)
{
	int res;
	int alsval_ch0;
	int alsval_ch1;
	...
	res = ltr559_als_read_raw(ltr559_obj->client, &ltr559_obj->als, (u16 *)&alsval_ch0, (u16 *)&alsval_ch1,(u16 *)&ratio);
	
    return snprintf(buf, PAGE_SIZE, "Ch0=%d, Ch1=%d, Ratio=%d, Lux=%d\n", alsval_ch0, alsval_ch1,ratio, ltr559_obj->als);    
...
static DRIVER_ATTR(raw_als,     0664, ltr559_show_raw_als,   NULL);
...
//add start by jiangchong for prox sensor calibration,2015\12\09  
static void check_prox_mean(unsigned int prox_mean)
{
	APS_FUN();
    printk("check prox mean begin!!!");
	if(prox_mean <= 50)
	{
		prox_threshold_hi_param = prox_mean +100;
		prox_threshold_lo_param = prox_mean +50;
	}
	else if(50 < prox_mean && prox_mean <= 100)
	{
        prox_threshold_hi_param = prox_mean +100;
		prox_threshold_lo_param = prox_mean +50;
	}
	else if(prox_mean > 100 && prox_mean <= 200)
	{
		prox_threshold_hi_param = prox_mean + 110;
		prox_threshold_lo_param = prox_mean + 60;
	}
	else if(prox_mean > 200 && prox_mean <= 300)
	{
		prox_threshold_hi_param = prox_mean + 120;
		prox_threshold_lo_param = prox_mean + 70;
	}
	else if(prox_mean > 300 && prox_mean <= 400)
	{
        prox_threshold_hi_param = prox_mean +130;
		prox_threshold_lo_param = prox_mean +80;
    }
    else if(prox_mean > 400 && prox_mean <= 1500)
	{
        prox_threshold_hi_param = prox_mean +150;
		prox_threshold_lo_param = prox_mean +100;
    }
    else
    {
        prox_threshold_hi_param = 2000;
...
static void set_threshold(int prox_mean)
{
	APS_FUN();
	printk("set threshold begin!!!");
    check_prox_mean(prox_mean);
    ...
    {
        //ltr559_ps_set_thres((u16)(atomic_read(&ltr559_obj->ps_thd_val_high)), (u16)(atomic_read(&ltr559_obj->ps_thd_val_low)));
		ltr559_ps_set_thres();
    }
}

static int ltr559_get_threshold(struct i2c_client *client)
{
	//APS_FUN();
   int i = 0;
   ...
   //bool ltr559_ps_original_power;
   //struct ltr559_priv *obj = i2c_get_clientdata(client);    

  //ltr559_ps_original_power = test_bit(CMC_BIT_PS, &obj->enable) ? (TRUE) : (FALSE);
   //if(FALSE == ltr559_ps_original_power)
   //{
   ...
   for(i = 0; i < 16; i++)
   {
     ltr559_ps_read(client, &ps_thredata);
	 mdelay(80);
	 ...
	 //APS_DBG("xjx ps_thredata = %d\n", ps_thredata);
   }
   
   ps_thredata_mean = ps_thredata_sum >> 4;
   APS_DBG("ps_thredata_min=%d,ps_thredata_max=%d\n", ps_thredata_min, ps_thredata_max);


    //ltr559_ps_original_power = test_bit(CMC_BIT_PS, &obj->enable) ? (TRUE) : (FALSE);
    //if(FALSE == TMD2771_ps_original_power)
    //{
    	//TMD2771_enable_ps_withoutWakeLock(client, 0);
	//}
...
static void prox_adjust_on(struct work_struct *work)
{
	APS_FUN();
	printk("prox adjust on begin!!!");
	ps_calibrating = true;
	prox_threshold_startup_mean = ltr559_get_threshold(ltr559_i2c_client);
	ps_calibrating = false;
	APS_LOG("%s,befor check saved_value prox_threshold_startup_mean=%d\n",__func__,prox_threshold_startup_mean);
		
	if(prox_factory_saved_value > 0)
	{
		if(prox_threshold_startup_mean > prox_factory_saved_value)
		{
			if((prox_threshold_startup_mean - prox_factory_saved_value) > 300)
			{
				prox_threshold_startup_mean = prox_factory_saved_value;
			}
		}
		else if (prox_threshold_startup_mean <= prox_factory_saved_value )
		{
			if((prox_factory_saved_value - prox_threshold_startup_mean) > 40)
			{
				prox_threshold_startup_mean = prox_factory_saved_value;
			}
		}		
	}
	set_threshold(prox_threshold_startup_mean);
	
	APS_LOG("%s,prox_factory_saved_value=%d,hi=%d,lo=%d\n",__func__,prox_factory_saved_value, prox_threshold_hi_param, prox_threshold_lo_param );
		
}

ssize_t alsps_read_proc(struct file *filp, char *buff, size_t len, loff_t *off)
{
	//printk("alsps read proc begin!!&&&&&\n");
	//APS_FUN();
    int cnt;
	ps_calibrating = true;
	prox_factory_saved_value = ltr559_get_threshold(ltr559_i2c_client);
	set_threshold(prox_factory_saved_value);
    printk("alsps read proc end!!$$$$$\n");
	cnt = sprintf(buff, "%d\n", prox_factory_saved_value);
	printk("jiangchongdebug*****\n");
	//cnt = prox_factory_saved_value;
	ps_calibrating = false;
	
    //*eof = 1;
    APS_DBG("%s,prox_factory_saved_value=%d,hi=%d,lo=%d\n",__func__,prox_factory_saved_value, prox_threshold_hi_param,prox_threshold_lo_param);
...
{
	//APS_FUN();
	//printk("alsps write proc begin!!&&&&&\n");
	//int ret = 0;
	int value = 0;
	sscanf(buff, "%d", &value);
	printk("jiangchongdebug======\n");
	prox_factory_saved_value = value;
	printk("jiangchongdebug+++++++\n");
	queue_delayed_work(ltr559_obj->adjust_wqueue, &ltr559_obj->prox_adjust_work, HZ/10); 
	printk("jiangchongdebug#######\n");
	//set_threshold(prox_factory_saved_value);
	APS_DBG("%s,prox_factory_saved_value=%d,hi=%d,lo=%d\n",__func__,prox_factory_saved_value, prox_threshold_hi_param,prox_threshold_lo_param);
...
static int ltr559_als_read(struct i2c_client *client, u16* data)
{
	...
	//reg = ltr559_i2c_read_reg(LTR559_ALS_PS_STATUS);
    //   APS_DBG("chenyingfang == reg = %x\n",reg);
	//if(0 == (reg & 0X04))
	//{
	//    luxdata_int = previous_lux_val;
	//}
...
//	{
		...
		//alsval_ch1 = (buffer[1]<<8)+buffer[0];
		//alsval_ch0 = (buffer[3]<<8)+buffer[2];	 
		...
		alsval_ch0_hi = buffer[3];
             //alsval_ch0_lo = buffer_alsval_ch0_lo;
		//alsval_ch0_hi = buffer_alsval_ch0_hi;	 
		alsval_ch0 = (alsval_ch0_hi * 256) + alsval_ch0_lo;
		//APS_DBG("alsval_ch0_lo = %d,alsval_ch0_hi=%d,alsval_ch0=%d\n",alsval_ch0_lo,alsval_ch0_hi,alsval_ch0);
...
static int ltr559_devinit(void)
{
	...
	res = ltr559_i2c_write_reg(LTR559_PS_N_PULSES, 3); 
	if(res<0)
	{
...
static struct file_operations ltr559_fops = {
	...
	//add start by jiangchong for prox sensor calibration,2015\12\09
	.read = alsps_read_proc,
	.write = alsps_write_proc,
	//add end by jiangchong for prox sensor calibration,2015\12\09
};
/*----------------------------------------------------------------------------*/
...
{
	//APS_FUN();
	//printk("create proc file begin!!!@@@!!\n");
	struct proc_dir_entry *alsps_proc_file; 
	alsps_proc_file = proc_create("driver/ltr559_threshold", 0646, NULL, &ltr559_fops);
	if(alsps_proc_file) 
	{
		//alsps_proc_file->read_proc = alsps_read_proc;
		//alsps_proc_file->write_proc = alsps_write_proc;
		//alsps_proc_file->write_proc = alsps_write_proc;
		printk("create proc file success!!\n");
	} 
	else
		printk("alsps proc file create failed!\n");
}
//add end by jiangchong for prox sensor calibration,2015\12\09
...
static int ltr559_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    //add start by jiangchong for prox sensor calibration,2015\12\09
	INIT_DELAYED_WORK(&obj->prox_adjust_work,prox_adjust_on);
	obj->adjust_wqueue = create_singlethread_workqueue("proximity_adjust");
	if(!obj->adjust_wqueue){
    ...
    //add start by jiangchong for prox sensor calibration,2015\12\09
    printk("create file begin!!\n");
    create_alsps_proc_file();
    printk("create file end!!\n");
    //add start by jiangchong for prox sensor calibration,2015\12\09
#if defined(CONFIG_HAS_EARLYSUSPEND)
