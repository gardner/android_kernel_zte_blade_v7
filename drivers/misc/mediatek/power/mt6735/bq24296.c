...
// add set ship mode by lixinghe, 20160126
...
static struct delayed_work  *set_shipmode_work=NULL;
static struct workqueue_struct *set_shipmode_workqueue=NULL;
static bool set_ship_mode_flag = false;
void bq24296_set_ship_mode(void)
{	
    bq24296_set_watchdog(0x0);
    bq24296_set_batfet_disable(0x1);        
    battery_log(BAT_LOG_CRTI, "set bq24296 shipmode done.\n");
}

static void set_shipmode_work_func(struct work_struct *work)
{
    mm_segment_t oldfs;
    long fd = 0;
    int nReadSize;
    char buf=0;
    char check = '0';
    char dev[]="/sys/class/power_supply/usb/online";

    battery_log(BAT_LOG_CRTI, "bq24296 lixinghe: %s start.\n", __func__);
	oldfs = get_fs();
	set_fs(KERNEL_DS);
	fd = sys_open(dev, O_RDONLY, 0);
	if (fd < 0) {
		battery_log(BAT_LOG_CRTI, "bq24296 lixinghe:open fail dev:%s n", dev);
        return;
	}
    
    if((nReadSize = sys_read(fd, &buf, 1)) != 1)
    {
		battery_log(BAT_LOG_CRTI, "bq24296 lixinghe: read fail dev:%s nReadSize:%d\n", dev, nReadSize);
        goto set_shipmode_read_failed;
	}
    set_fs(oldfs);
    
    battery_log(BAT_LOG_CRTI, "bq24296 lixinghe: %s buf is %c.\n", __func__, buf);
        
    if(strncmp(&buf, &check, 1) == 0)
    {
        if(set_ship_mode_flag == false)
        {
            set_ship_mode_flag = true;
            battery_log(BAT_LOG_CRTI, "bq24296 lixinghe: %s bq24296_set_ship_mode.\n", __func__);

//            kfree(set_shipmode_work);
//          destroy_workqueue(set_shipmode_workqueue);
            bq24296_set_ship_mode();
        }
    }
    else
    {
        battery_log(BAT_LOG_CRTI, "bq24296 lixinghe: %s queue_delayed_work.\n", __func__);
        queue_delayed_work(set_shipmode_workqueue, set_shipmode_work, msecs_to_jiffies(300)); 
    }    

set_shipmode_read_failed:
    sys_close(fd);
}

static ssize_t attrset_bq24296_shipmode(struct device *dev, struct device_attribute *attr,
				    const char *buf, size_t size)
{
	...
	if(strncmp(buf, &check, 1) == 0)
	{
        if( (set_shipmode_work = kzalloc(sizeof(*set_shipmode_work), GFP_KERNEL)) == NULL)
            return -1;
        INIT_DELAYED_WORK(set_shipmode_work, set_shipmode_work_func);
        if((set_shipmode_workqueue = create_singlethread_workqueue("set_shipmode")) == NULL)
            return -2;
        battery_log(BAT_LOG_CRTI, "bq24296 lixinghe: queue_delayed_work.\n");
        queue_delayed_work(set_shipmode_workqueue, set_shipmode_work, 0);
        
	}
	return 1;
}

static ssize_t attrget_bq24296_shipmode(struct device *dev, struct device_attribute *attr, char *buf)
{
	
	return sprintf(buf, "no ship mode\n");
}
static DEVICE_ATTR(bq24296_access, 0664, show_bq24296_access, store_bq24296_access);	/* 664 */
static DEVICE_ATTR(bq24296_shipmode, 0664, attrget_bq24296_shipmode, attrset_bq24296_shipmode);	

// add set ship mode by lixinghe, 20160126 end
...
static int bq24296_user_space_probe(struct platform_device *dev)
{
	...
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_bq24296_shipmode); // add set ship mode by lixinghe, 20160126
