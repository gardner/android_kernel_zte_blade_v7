...
static void lcm_id_detect(void)
{
	...
	if((pin_lcd_id0==0x0)&&(pin_lcd_id1==0x0))
	     module_name = 1;//LEAD
	else if((pin_lcd_id0==0x1)&&(pin_lcd_id1==0x1))
	     module_name = 2;//TXD
}
//	ssize_t (*read) (struct file *, char __user *, size_t, loff_t *);
static ssize_t  mdss_dsi_panel_lcd_read_proc(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{	
	int len = 0;
	char *page = NULL;

	page = kmalloc(64, GFP_KERNEL);	
	if (!page) 
	{		
		kfree(page);		
		return -ENOMEM;	
	...
	if(module_name == 1)	
	len = sprintf(page, "%s\n", "zfgLEAD_hx8399c_1080*1920_5.2Inch");		
	else if(module_name == 2)
	len = sprintf(page, "%s\n", "zfgJDF_nt35596_1080*1920_5.2Inch");
	if (copy_to_user((void __user *)buffer,(void *)page,len)) 			
 	{
		DISPERR("create_proc_entry msm_lcd copy_to_user failed\n");
		return -1;
	}
...
static const struct file_operations msm_lcd_proc_fops = { 
    .owner = THIS_MODULE,
...
{	
		...
		d_entry = proc_create("msm_lcd", 0644, NULL, &msm_lcd_proc_fops);
		if (d_entry== NULL)
		{
			DISPERR("create_proc_entry msm_lcd failed\n");
		}
/*		
